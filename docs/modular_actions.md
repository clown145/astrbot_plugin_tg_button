...
4.  **自动生效**：
    > [!TIP]
    > **通过 WebUI 上传动作后会自动加载，无需手动重载插件！**
    > 上传成功后，您的新动作会立即出现在 WebUI 的动作列表和工作流编辑器中，可以直接使用。



### 方法 B：手动创建文件 (推荐用于本地开发)

如果您熟悉服务器文件系统操作，也可以直接在服务器上创建和编辑动作文件。这种方法的好处是可以使用您喜欢的 IDE 进行开发。

> [!IMPORTANT]
> **手动修改文件后需要重载插件生效**：
> 与 WebUI 上传不同，直接在磁盘上修改 `.py` 文件后，插件不会自动检测到变化。您需要在 AstrBot 的主 WebUI 中找到“插件管理”，然后点击本插件旁边的“重载”按钮来刷新动作列表。
...
- **通过 WebUI 上传/更新**：后台会自动执行重新扫描，**立即生效**，无需重载插件。
- **直接在服务器上修改文件**：由于插件无法实时监控文件系统变化，您需要**重启 AstrBot 或在插件管理中点击“重载”**。

生效后，您就可以在侧边栏的“工作流编辑器”中找到并使用您刚刚创建的动作了。建议先在“模块动作”页面确认该动作已正确加载（状态显示为正常）。
