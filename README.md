...
WebUI 拥有强大的功能，可以配置能够执行服务器端逻辑的“动作 (Action)”按钮。因此，保护对 WebUI 的访问至关重要。

> [!WARNING]
> **强烈建议为 WebUI 设置密钥！**
> 如果您需要长期开启 WebUI，特别是当您将端口暴露给公网（`webui_host` 设置为 `0.0.0.0`）时，**必须**在插件配置中设置一个长而复杂的 `webui_auth_token`。
